...
beforeEach(function () {
  ...
  firstThumb[0].classList.add("thumb_first");
...
describe("ProgressBar", () => {
  it("Инициализация ProgressBar", () => {
    ...
    expect(bar.slider).toContainElement("div.progress-bar");
    expect(bar.progressBar).toBeInstanceOf(HTMLElement);
    expect(bar.progressBar).toHaveClass("progress-bar");
  });
  describe("метод addBar", () => {
    let thumbFirst: HTMLElement;
    let thumbSecond: HTMLElement;
    beforeEach(function () {
      thumbFirst = bar.slider.querySelector(".thumb_first") as HTMLElement;
      thumbSecond = bar.slider.querySelector(".thumb_second") as HTMLElement;
    });
  ...
  describe("метод checkOrientation", () => {
    it("при orientation = vertical устанавливает блоку класс progress-bar_vertical и удаляет класс progress-bar_horizontal", () => {
      ...
      expect(bar.progressBar).toHaveClass("progress-bar_vertical");
      expect(bar.progressBar).not.toHaveClass("progress-bar_horizontal");
    });
    it("при orientation = horizontal устанавливает блоку класс progress-bar_horizontal и удаляет класс progress-bar_vertical", () => {
      bar.config.orientation = "horizontal";
      bar.checkOrientation();
      expect(bar.progressBar).not.toHaveClass("progress-bar_vertical");
      expect(bar.progressBar).toHaveClass("progress-bar_horizontal");
    });
  });
  ...
  it("метод updateBarConfig обновляет конфиг и вызывает ф-ю checkOrientation", () => {
    ...
    expect(bar.checkOrientation).toHaveBeenCalled();
  });
});
