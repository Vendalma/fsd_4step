...
describe('ProgressBar', () => {
	...
	describe('проверка метода setOnloadProgressBarPosition', ()=> {
		...
		it('orientation = horisontal, range = false', () => {
			...
			expect(ProgressBar.progressBar).toHaveCss({height : '7px', left : '0px'})
			expect(ProgressBar.progressBar).toHaveCss({width: onloadPositionThumbOne + 2 +'px'})
		})
		it('orientation = horisontal, range = true', () => {
			...
			expect(ProgressBar.progressBar).toHaveCss({height : '7px'})
			expect(ProgressBar.progressBar).toHaveCss({left : onloadPositionThumbOne + "px"})
			expect(ProgressBar.progressBar).toHaveCss({width: onloadPositionThumbTwo - onloadPositionThumbOne + "px"})
		})
		it('orientation = vertical, range = false', () => {
			...
			expect(ProgressBar.progressBar).toHaveCss({width : '7px', top : "0px"})
			expect(ProgressBar.progressBar).toHaveCss({height: onloadPositionThumbOne + 2 +"px"})
		})
		it('orientation = vertical, range = true', () => {
			...
			expect(ProgressBar.progressBar).toHaveCss({width : '7px', left: '0px'})
			expect(ProgressBar.progressBar).toHaveCss({top : onloadPositionThumbOne + "px"})
			expect(ProgressBar.progressBar).toHaveCss({height: onloadPositionThumbTwo - onloadPositionThumbOne + "px"})
		})
	})
	describe('проверка метода setPositionForThumbOne', ()=> {
		...
		beforeAll(function () {
			position = 100;
			secondThumb = ProgressBar.slider.querySelector(".thumb_second") as HTMLElement;	
		})
		it('orientation = horisontal, range = false', () => {
			ProgressBar.range = false;
			ProgressBar.orientation = 'horisontal'
		...
		it('orientation = horisontal, range = true', () => {
				...
				expect(ProgressBar.progressBar).toHaveCss({width: parseInt(secondThumb.style.left) - position + "px"})
			})
			it('orientation = vertical, range = false', () => {
				ProgressBar.range = false;
				ProgressBar.orientation = 'vertical'
				...
				expect(ProgressBar.progressBar).toHaveCss({height: position + 2 +"px"})
			})
			it('orientation = vertical, range = true', () => {
				ProgressBar.range = true;
				ProgressBar.orientation = 'vertical'
				...
				expect(ProgressBar.progressBar).toHaveCss({height: parseInt(secondThumb.style.top) - position + "px"})
			})
		})
	describe('проверка метода setPositionForThumbTwo', ()=> {
		...
		it('orientation = horisontal', ()=> {
			...
			expect(ProgressBar.progressBar).toHaveCss({width: position - parseInt(ProgressBar.progressBar.style.left) + 7 + "px"})
		})
		it('orientation = vertical', ()=> {
			ProgressBar.orientation = 'vertical';
			ProgressBar.setPositionForThumbTwo(position);
			expect(ProgressBar.progressBar.style.height).toEqual('229px')
			expect(ProgressBar.progressBar).toHaveCss({height: position - parseInt(ProgressBar.progressBar.style.top) + 5 + "px"})
		})
	})
})