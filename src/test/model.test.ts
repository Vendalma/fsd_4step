...
describe("Model", () => {
  ...
  describe("метод updateConfig обновляет конфиг модели", () => {
    ...
    it("если в принимаемом объекте key = label, тто вызываются ф-ция  broadcast класса Observer", () => {
      const data = {
        label: false,
      };
      model.updateConfig(data);
  ...
  describe("метод calcPositionTo", () => {
    it("range = true, positionTo <= positionFrom", () => {
      spyOn(model, "calcPositionFrom");
  ...
  it("метод calcPixelSize возвращает кол-во пикселей для одного шага", () => {
    ...
    model.sliderSize = 40;
    const expectResult =
      (model.config.max - model.config.min) / model.sliderSize;
  ...
  it("метод calcValue возвращает число", () => {
    ...
    const expectedResult =
      Math.round((400 * 0.25 + model.config.min) / model.config.step) *
      model.config.step;
